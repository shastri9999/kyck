...
class SelectFormController {
    constructor($rootScope, BrokerageResource, $mdDialog, $mdToast, $mdStepper, CalendarService, moment, AuthenticationService) {
        'ngInject';
        ...
        function processEvents(data) {
            var userSlots = data.map(function (a) {
                return moment(a['startTime'], 'DD/MM/YYYY hh:mm').toDate();
            }).reverse();
            var ctr = 0;
            var userSlot = userSlots[ctr];
            for (var j = 0; j <= 150; ++j) {
                ...
                if (day % 7 != 6 && day % 7 != 0) {
                    for (var i = 10; i <= 19; ++i) {
                        ...
                        if (startD > userSlot && ctr < userSlots.length) {
                            userSlot = userSlots[ctr];
                            ctr += 1;
                        }
                        if (startD != userSlot) {
                            vm.events.push({
        ...
        if (AuthenticationService.isBroker()) {
        ...
        function eventClicked($selectedEvent, index) {
            ...
            $rootScope.$on('submitBrokerage', function ($event, e) {
                ...
                BrokerageResource.submitBrokerageApplication({}, {
                    "brokerageCalenderSlot": brokerageReqs
                }, function (s) {
                ...
                if (!vm.dialogShown) {
                    ...
                    $mdDialog.show({
                        parent: parentEl
                        , targetEvent: $event
                        , template: '<md-dialog aria-label="List dialog">' + '  <md-dialog-content style="width:500px; max-width: 90%; min-height:60px;">' + '<div class="dialog-content-broker">' + ' Your appointment preferences have been sent to the partners.' + ' </div>' + '  </md-dialog-content>' + '  <md-dialog-actions>' + '    <md-button ng-click="closeDialog()" class="md-primary">' + '      Okay!' + '    </md-button>' + '  </md-dialog-actions>' + '</md-dialog>'
